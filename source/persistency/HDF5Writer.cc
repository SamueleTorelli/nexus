...
HDF5Writer::HDF5Writer():
  ...
  ipart_(0), ipos_(0), istep_(0), igamma_(0)
{
}
...
void HDF5Writer::Open(std::string fileName, bool debug)
{
  ...
  memtypeGammaInteraction_ = createGammaInteractionType();
...
void HDF5Writer::WriteGammaInteraction(int64_t evt_number, int particle_id, const char* particle_name, char primary, int mother_id, float initial_x, float initial_y, float initial_z, float initial_t, float final_x, float final_y, float final_z, float final_t, const char* initial_volume, const char* final_volume, float initial_momentum_x, float initial_momentum_y, float initial_momentum_z, float final_momentum_x, float final_momentum_y, float final_momentum_z, float kin_energy, float length, const char* creator_proc, const char* final_proc)
{
  gamma_interaction_t gamma;
  gamma.event_id = evt_number;
  gamma.particle_id = particle_id;
  memset(gamma.particle_name, 0, STRLEN);
  strcpy(gamma.particle_name, particle_name);
  gamma.primary = primary;
  gamma.mother_id = mother_id;
  gamma.initial_x = initial_x;
  gamma.initial_y = initial_y;
  gamma.initial_z = initial_z;
  gamma.initial_t = initial_t;
  gamma.final_x = final_x;
  gamma.final_y = final_y;
  gamma.final_z = final_z;
  gamma.final_t = final_t;
  memset(gamma.initial_volume, 0, STRLEN);
  strcpy(gamma.initial_volume, initial_volume);
  memset(gamma.final_volume, 0, STRLEN);
  strcpy(gamma.final_volume, final_volume);
  gamma.initial_momentum_x = initial_momentum_x;
  gamma.initial_momentum_y = initial_momentum_y;
  gamma.initial_momentum_z = initial_momentum_z;
  gamma.final_momentum_x = final_momentum_x;
  gamma.final_momentum_y = final_momentum_y;
  gamma.final_momentum_z = final_momentum_z;
  gamma.kin_energy = kin_energy;
  gamma.length = length;
  memset(gamma.creator_proc, 0, STRLEN);
  strcpy(gamma.creator_proc, creator_proc);
  memset(gamma.final_proc, 0, STRLEN);
  strcpy(gamma.final_proc, final_proc);

  writeGammaInteraction(&gamma, gammaInteractionTable_, memtypeGammaInteraction_, igamma_);
